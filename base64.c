 ...
 * Copyright (c) 2019, Christian E. Häußler <chrissx@chrissx.ga>
 *
...
char *base64_encode(char *src, size_t len, size_t *out_len)
{
	...
	if (olen < len) return NULL; // overflow
	out = malloc(olen);
...
char *base64_decode(char *src, size_t len, size_t *out_len)
{
	...
	count = 0;
	for (i = 0; i < len; i++)
		if ((unsigned)dtable[(unsigned)src[i]] != 0x80)
			count++;
	...
	count = 0;
	for (i = 0; i < len; i++) {
		tmp = dtable[(unsigned)src[i]];
		if ((unsigned)tmp == 0x80) continue;
		if (src[i] == '=') pad++;
		block[count] = tmp;
		count++;
		if (count == 4) {
